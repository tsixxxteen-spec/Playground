...
SPRINT_ID="21B.8-EDITOR-WIRING"
MARKER=".playground-sprint-21B8-editor-wiring-installed"
BACKUP_DIR=".playground-backups/sprint-21B8-editor-wiring-$(date +%Y%m%d-%H%M%S)"
...
[[ -f src/collaboration/mutations/events.ts ]] || fail "Sprint 21B.8 mutation events were not found."
[[ -f src/collaboration/mutations/types.ts ]] || fail "Sprint 21B.8 mutation types were not found."
[[ -f src/collaboration/mutations/SharedMutationBridge.tsx ]] || fail "SharedMutationBridge was not found."
...
rollback() {
  ...
  if [[ $code -ne 0 ]]; then
    ...
    rm -f \
      src/collaboration/mutations/EditorMutationAdapter.tsx \
      "$MARKER"
...
# ------------------------------------------------------------
# Automatic editor mutation adapter
# ------------------------------------------------------------

cat > src/collaboration/mutations/EditorMutationAdapter.tsx <<'EOF'
import {
  useEffect,
} from "react";
...
text = insert_import(
    text,
    (
        'import EditorMutationAdapter '
        'from "./collaboration/mutations/EditorMutationAdapter";\n'
    ),
    'from "./collaboration/mutations/EditorMutationAdapter"',
)

if "<EditorMutationAdapter />" not in text:
    bridge_anchor = "<SharedMutationBridge />"

    if bridge_anchor in text:
        text = text.replace(
            bridge_anchor,
            (
        ...
        if app_anchor not in text:
            raise SystemExit(
                "❌ Neither SharedMutationBridge nor <App /> was found in src/main.tsx."
            )
...
print("✅ EditorMutationAdapter imported.")
print("✅ EditorMutationAdapter mounted.")
PY
...
echo "  $BACKUP_DIR"
echo ""
echo "Automatic wiring enabled for:"
echo "  • movement"
echo "  • resizing"
echo "  • style and property changes"
echo "  • deletion"
echo ""
echo "Required object markup:"
echo '  data-playground-object-id={object.id}'
echo ""
echo "Optional precise geometry attributes:"
echo "  data-playground-x"
echo "  data-playground-y"
echo "  data-playground-width"
echo "  data-playground-height"
echo "  data-playground-properties"
echo ""
echo "Test:"
echo "  1. Run npm run dev"
echo "  2. Open the same session in two tabs"
echo "  3. Move or resize an identified object"
echo "  4. Confirm the second tab updates"
echo "  5. Change an object property"
echo "  6. Delete an object and confirm synchronized removal"
