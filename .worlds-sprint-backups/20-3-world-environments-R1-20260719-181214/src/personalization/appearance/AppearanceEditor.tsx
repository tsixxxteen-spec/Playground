...
const tabs: Array<{id: AppearanceTab; label: string}> = [
  {id:"themes",label:"Themes"},{id:"decorations",label:"Decorations"},{id:"objects",label:"Objects"},{id:"widgets",label:"Widgets"},{id:"effects",label:"Effects"},
];
export default function AppearanceEditor({ value, onChange }: Props) {
  ...
  return <section className="appearance-editor">
    ...
    <div className="appearance-editor__body">
      {tab === "themes" ? <ThemeSelector value={value.themeId} onChange={(themeId) => onChange({ ...value, themeId })} /> : null}
      ...
      {tab !== "themes" && tab !== "objects" ? <div className="appearance-editor__coming-soon"><span>{tab.toUpperCase()}</span><h4>Studio foundation ready</h4><p>This section is reserved for the next focused sprint.</p></div> : null}
    </div>
  </section>;
}
