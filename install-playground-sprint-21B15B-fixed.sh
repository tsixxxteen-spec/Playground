...
BACKUP_DIR=".playground-backups/sprint-21B15B-fixed-$(date +%Y%m%d-%H%M%S)"
...
export PLAYGROUND_INSTALL_VERSION="$VERSION"
export PLAYGROUND_INSTALL_BUILD_DATE="$BUILD_DATE"
export PLAYGROUND_INSTALL_BUILD_NUMBER="$BUILD_NUMBER"
...
version = os.environ["PLAYGROUND_INSTALL_VERSION"]
build_date = os.environ["PLAYGROUND_INSTALL_BUILD_DATE"]
build_number = os.environ["PLAYGROUND_INSTALL_BUILD_NUMBER"]
...
if "__PLAYGROUND_VERSION__" in source:
    ...
    define_block = (
        ...
        f'{json.dumps(define_values["__PLAYGROUND_VERSION__"])},\n'
        f'    __PLAYGROUND_BUILD_DATE__: '
        f'{json.dumps(define_values["__PLAYGROUND_BUILD_DATE__"])},\n'
        f'    __PLAYGROUND_BUILD_NUMBER__: '
        f'{json.dumps(define_values["__PLAYGROUND_BUILD_NUMBER__"])},\n'
        "  },\n"
    )
...
diagnostics_replacement = '''      {PLAYGROUND_RUNTIME_CONFIG
        .diagnostics
        .mountPanel && (
...
readiness_replacement = '''      {PLAYGROUND_RUNTIME_CONFIG
        .releaseReadiness
        .mountPanel && (
