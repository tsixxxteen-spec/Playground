...
function WorldCanvasEditor({
  ...
  const snapPosition = useCallback((position: PlaygroundPosition) => {
    const normalized = normalizePlaygroundPosition(position);
    if (!snapEnabled) return normalized;
    return {
      x: Math.round(normalized.x / gridSize) * gridSize,
      y: Math.round(normalized.y / gridSize) * gridSize,
    };
  ...
  const changeLayer = useCallback((objectId: string, direction: "forward" | "backward") => {
    const ordered = [...draft.objects].sort((a, b) => a.zIndex - b.zIndex);
    ...
    executeCommand(createUpdateObjectCommand(objectId, (object) => ({ ...object, zIndex: other.zIndex })));
    executeCommand(createUpdateObjectCommand(other.id, (object) => ({ ...object, zIndex: selected.zIndex })));
  }, [draft.objects, executeCommand]);

  const centerObject = useCallback((objectId: string, axis: "x" | "y") => {
    updateObject(objectId, (object) => ({
      ...object,
      position: { ...object.position, [axis]: 50 },
    }));
  }, [updateObject]);

  useEffect(() => {
    if (mode !== "edit" || !selectedObjectId) return undefined;
    const handleNudge = (event: KeyboardEvent) => {
      if (isEditableShortcutTarget(event.target)) return;
      const deltas: Record<string, PlaygroundPosition> = {
        ArrowLeft: { x: -1, y: 0 },
        ArrowRight: { x: 1, y: 0 },
        ArrowUp: { x: 0, y: -1 },
        ArrowDown: { x: 0, y: 1 },
      ...
      if (!delta) return;
      const selected = draft.objects.find((object) => object.id === selectedObjectId);
      if (!selected) return;
      event.preventDefault();
      const multiplier = event.shiftKey ? 5 : 1;
      moveObject(selected.id, {
        x: selected.position.x + delta.x * multiplier,
        y: selected.position.y + delta.y * multiplier,
      });
    };
    window.addEventListener("keydown", handleNudge);
    return () => window.removeEventListener("keydown", handleNudge);
  }, [draft.objects, mode, moveObject, selectedObjectId]);
  ...
  return (
      <div
        ...
        style={{
          ...
          overflow: "hidden",
          pointerEvents: "none",
          backgroundImage: mode === "edit" && snapEnabled
            ? "linear-gradient(rgba(255,255,255,.055) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.055) 1px, transparent 1px)"
            : undefined,
          backgroundSize: mode === "edit" && snapEnabled ? `${gridSize}% ${gridSize}%` : undefined,
        }}
      >
        {objects.map((object) => (
          <WorldObject
        ...
        {mode === "edit" ? (
          <>
            ...
            <WorldInspector
              ...
              onBringForward={(id) => changeLayer(id, "forward")}
              onSendBackward={(id) => changeLayer(id, "backward")}
              onCenterHorizontal={(id) => centerObject(id, "x")}
              onCenterVertical={(id) => centerObject(id, "y")}
            />
          </>
        ) : null}
      </div>
