...
function validateManifest(value: unknown): ThemeManifest {
  if (!value || typeof value !== "object") {
    throw new Error("The selected file does not contain a valid manifest.");
  }
  ...
  if (
    ...
    manifest.type !== "playground-theme-manifest" ||
    !manifest.theme
  ) {
    throw new Error("This is not a supported Playground theme manifest.");
  }
  ...
  if (
    ...
    (theme.tier !== "Included" && theme.tier !== "Premium")
  ) {
    throw new Error("The manifest is missing required theme information.");
  }

  if (!isSemver(theme.version)) {
    throw new Error("Theme version must use semantic versioning, such as 1.0.0.");
  }

  const accent = theme.colors?.accent;
  if (accent && !isHexColor(accent)) {
    throw new Error("The manifest accent color is invalid.");
  }
...
function downloadLibrary(drafts: CreatorDraft[]) {
  ...
  anchor.href = url;
  anchor.download = "playground-creator-drafts.json";
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  URL.revokeObjectURL(url);
...
export default function CreatorStudio({ open, onClose }: Props) {
  ...
  const canSave = useMemo(
    () =>
      ...
      isSemver(version) &&
      isHexColor(accent),
    [accent, description, name, version],
  );
  ...
  const saveDraft = () => {
    if (!canSave) {
      setMessage(
        "Add a name, fuller description, valid hex color, and version like 1.0.0.",
      );
      return;
    }
  ...
  const importManifest = async (
    ...
    try {
      ...
      const draft: CreatorDraft = {
        id: crypto.randomUUID(),
        name: manifest.theme.name.trim(),
        category: manifest.theme.category.trim(),
        description: manifest.theme.description.trim(),
        tier: manifest.theme.tier,
        accent: manifest.theme.colors?.accent ?? "#8f7cff",
        version: manifest.theme.version.trim(),
        createdAt: new Date().toISOString(),
      };

      persist([draft, ...drafts]);
      setMessage(`Imported “${draft.name}” as a local draft.`);
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "The selected manifest could not be imported.",
      );
    }
  };

  const duplicateDraft = (draft: CreatorDraft) => {
    const copy: CreatorDraft = {
      ...draft,
      id: crypto.randomUUID(),
    ...
    persist([copy, ...drafts]);
    setMessage(`Duplicated “${draft.name}”.`);
  };
      ...
      <section className="creator-studio__panel">
        ...
        <div className="creator-studio__body">
          ...
          >
            ...
            <div className="creator-studio__primary-actions">
              <button
                className="creator-studio__secondary"
                type="button"
            ...
            <input
              ref={importRef}
              className="creator-studio__file-input"
              type="file"
          ...
          <aside className="creator-studio__drafts">
            <div className="creator-studio__draft-heading">
              <div>
              ...
              <button
                type="button"
                onClick={() => downloadLibrary(drafts)}
        ...
        <footer className="creator-studio__footer">
          Local creator packaging is complete. Publishing, review, payouts,
          and license verification require the future marketplace backend.
        </footer>
      </section>
    </div>
  );
