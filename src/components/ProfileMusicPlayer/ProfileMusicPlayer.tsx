...
export default function ProfileMusicPlayer({
  ...
  const normalized = useMemo(
    () => normalizeSoundtrack(soundtrack),
    [soundtrack],
  );

  const playable = useMemo(
    () =>
      normalized.tracks.filter(
        (track) =>
          track.sourceType === "upload" &&
          Boolean(track.source),
      ),
    [normalized.tracks],
  );
  ...
  const [expanded, setExpanded] = useState(
    hiddenMode ? false : defaultExpanded,
  );
  ...
  useEffect(() => {
    const onMediaPlay = (event: Event) => {
      ...
      if (
        target.dataset.profileSoundtrack === "true" ||
        target.muted ||
        target.volume <= 0
      ) {
        return;
  ...
  if (!visible || normalized.tracks.length === 0) {
    return null;
  }
  ...
  const previous = () => {
    ...
    setIndex(
      (value) =>
        (value - 1 + playable.length) % playable.length,
    );
  };
  ...
  const choose = (track: SoundtrackTrack) => {
    ...
    const nextIndex = playable.findIndex(
    ...
    <div className={className}>
      <header className="profile-soundtrack__header">
        ...
        >
          ...
          <span>
            <strong>
              {current?.title ?? "Profile soundtrack"}
            </strong>

            <small>
              {current?.artist ??
                `${normalized.tracks.length} tracks`}
            </small>
          </span>
        </button>
      ...
      <div
        className="profile-soundtrack__body"
        aria-hidden={!expanded && !hiddenMode}
      >
        <div className="profile-soundtrack__controls">
          ...
          <button
            className="profile-soundtrack__play"
            type="button"
            onClick={toggle}
            disabled={!current}
            aria-label={
        ...
        <div className="profile-soundtrack__playlist">
          {normalized.tracks.map((track, trackIndex) => (
            <button
              type="button"
              key={track.id}
              data-active={current?.id === track.id}
