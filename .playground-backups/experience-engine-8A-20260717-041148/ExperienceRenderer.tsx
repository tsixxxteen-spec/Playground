...
function Music({
  ...
  if (!soundtrack.tracks.length || (!visible && !hiddenAutoplay)) return null;
  return (
    <section className={`xp-music xp-music--${variant}`}>
      ...
      <ProfileMusicPlayer
        ...
        defaultExpanded={!hiddenAutoplay && (variant === "graphite" || variant === "release")}
      />
    </section>
  );
}
    ...
    <main className="xp xp--ivory">
      ...
      <header className="xp-ivory__hero">
        ...
        <div>
          ...
          <Music soundtrack={props.soundtrack} visible={props.showMusicPlayer} hiddenAutoplay={props.hiddenAutoplay} variant="editorial" label="Now playing" />
        </div>
        <Stats {...props} />
      </header>
  ...
  return (
    <main className="xp xp--contact">
      <aside className="xp-contact__rail">
        ...
        <Stats {...props} />
        <Music soundtrack={props.soundtrack} visible={props.showMusicPlayer} hiddenAutoplay={props.hiddenAutoplay} variant="utility" />
  ...
  return (
    <main className="xp xp--graphite">
      ...
      <header className="xp-graphite__hero">
        ...
        <Avatar profile={props.profile} />
        <Music soundtrack={props.soundtrack} visible={props.showMusicPlayer} hiddenAutoplay={props.hiddenAutoplay} variant="graphite" label="Featured audio" />
      </header>
...
function EightBit(props: Props) {
  return (
    <main className="xp xp--eight-bit">
      ...
      <Music soundtrack={props.soundtrack} visible={props.showMusicPlayer} hiddenAutoplay={props.hiddenAutoplay} variant="pixel" label="MUSIC_PLAYER.EXE" />
    </main>
  );
}
    ...
    <main className="xp xp--stardust">
      ...
      <div className="xp-star__layout">
        <aside className="xp-star__side">
          ...
          {props.profile.bio && <blockquote>{props.profile.bio}</blockquote>}
          <Stats {...props} />
          <Music soundtrack={props.soundtrack} visible={props.showMusicPlayer} hiddenAutoplay={props.hiddenAutoplay} variant="stardust" label="♫ cassette deck" />
    ...
    <main className="xp xp--press">
      ...
      <header className="xp-press__hero">
        ...
        <div className="xp-press__copy">
          ...
          <h1>{props.profile.displayName}</h1>
          {props.profile.bio && <h2>{props.profile.bio}</h2>}
          <Music soundtrack={props.soundtrack} visible={props.showMusicPlayer} hiddenAutoplay={props.hiddenAutoplay} variant="release" label="Featured release" />
          <Stats {...props} />
        </div>
      </header>
      <Posts count={props.postCount} label="Selected work">{props.posts}</Posts>
